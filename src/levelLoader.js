...
import { lightOn, logObject } from './main.js';
...
export class loadLevel {
    ...
    level1(scene) {
        ...
        if(currentLevel === 6){
            youFinishedAudio();
            logObject.generateLogs();
            
        }
        currentLevel = 1;
        logObject.startTimer();
        scene.remove(root);
        const glftLoader = new GLTFLoader();
    ...
    createVictoryBoundingBox(levelNumber) {
        if(levelNumber === 0){
            ...
            material = new THREE.MeshBasicMaterial( {color: 0x00ff00, transparent: true, opacity: 0.0} );
            if(lightOn){
