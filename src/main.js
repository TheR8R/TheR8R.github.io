...
import { loadLevel, material } from './levelLoader.js';
import { playerClass } from './player.js';
import { checkForCollision } from './collision.js';
import { logger} from './logger.js';

let camera, scene, renderer, controls, raycasters;
let prevTime = performance.now();
export let level;
export let player;
export let logObject = new logger();
let light = new THREE.HemisphereLight( 0xeeeeff, 0x777788, 2.5 );
...
function animate() {
    ...
    if ( controls.isLocked === true ) {
        ...
        movement(controls, delta, camera); // this function is in the keyboardControls.js file
